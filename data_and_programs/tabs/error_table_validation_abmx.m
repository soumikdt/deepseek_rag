...
save('forecast_validation_abmx','forecast','actual');
rmse_validation_abmx=100*squeeze(sqrt(nanmean((forecast-actual).^2)));
bias_validation_abmx=squeeze(nanmean(forecast-actual));
error_validation_abmx=forecast-actual;
...
input.data=cellstr(string(round(100*(rmse_validation_varx-rmse_validation_abmx)./rmse_validation_varx,1)));

for j=1:length(horizons)
    h=horizons(j);
    for l=1:number_variables
        [~,p_value]=dmtest_modified(rmmissing(error_validation_varx(:,j,l)),rmmissing(error_validation_abmx(:,j,l)),h);
...
fid=fopen('rmse_validation_abmx.tex','w');
...
input.data=cellstr(string(round(bias_validation_abmx,4)));

for j=1:length(horizons)
    h=horizons(j);
    for l=1:number_variables
        [~,~,p_value]=mztest(rmmissing(actual(:,j,l)),rmmissing(error_validation_abmx(:,j,l)+actual(:,j,l)));
        input.data{j,l}=[input.data{j,l},' (',num2str(round(p_value,2),'%.2f'),stars(p_value),')'];
    end
end
...
% save LaTex code as file
fid=fopen('bias_validation_abmx.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
