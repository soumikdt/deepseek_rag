...
input.data=cellstr(string(round(100*(rmse_var_nace10-rmse_abm_nace10)./rmse_var_nace10,1)));

for j=1:length(horizons)
    h=horizons(j);
    for l=1:number_variables
        [~,p_value]=dmtest_modified(rmmissing(error_var_nace10(:,j,l)),rmmissing(error_abm_nace10(:,j,l)),h);
...
fid=fopen('rmse_abm_nace10_var.tex','w');
